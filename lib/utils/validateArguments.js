...
function validateArguments(args, program) {
  if (args.length === 0) {
    ...
    logger.log(
      'Please try "crcf componentName" or "create-react-component-folder componentName"'
    )
    return false
  }
  ...
  if (program.typescript && program.reactnative) {
    logger.log()
    logger.error(
      'You cannot have a React Native and Typescript option at the same time'
    )
    return false
  }
  ...
  if (program.jsx && program.reactnative) {
    logger.log()
    logger.error(
      'You cannot have a React Native and JSX option at the same time'
    )
    return false
  }
